...
async def scrape_ig(ctx, tag, limit=15):
    ...
    reel_urls = []  # list of (alt_text, full_url)
    try:
        await page.goto(
        ...
        for el in links[:limit]:
            ...
            full = fu("https://www.instagram.com", href)
            reel_urls.append((alt[:200], full))
    except: pass
    finally:
        try: await page.close()
        except: pass

    # Open each reel page for real metrics — exact pattern from scrape_reels_to_excel.py
    for alt, url in reel_urls[:limit]:
        rp = await ctx.new_page()
        views = likes = None
        title = alt  # use alt as fallback title
        thumb = None
        desc_text = None
        creator = None
        posted_on = None
        try:
            ...
            # JSON fallbacks for views/likes
            if not views:
                for pat in [r'"video_view_count"\s*:\s*(\d+)', r'"play_count"\s*:\s*(\d+)']:
            ...
            # Creator
            m = re.search(r'"username"\s*:\s*"([^"]{2,40})"', html)
            if m: creator = "@" + m.group(1)

            # Upload timestamp
            m = re.search(r'"taken_at"\s*:\s*(\d{10})', html)
            if not m: m = re.search(r'"taken_at_timestamp"\s*:\s*(\d{10})', html)
            if m:
                try: posted_on = datetime.fromtimestamp(int(m.group(1))).isoformat()
                except: pass

            # Title from <title> — strip notification count AND platform suffix
            t_tag = re.search(r"<title>([^<]+)</title>", html, re.I)
            if t_tag:
                t = re.sub(r"^\(\d+\)\s*", "", t_tag.group(1))
                t = re.sub(r"\s*[•·|]\s*Instagram.*$", "", t, flags=re.I).strip()
                if len(t) > 5: title = t[:200]

            # Thumbnail
            m = re.search(r'og:image[^>]*content="([^"]*)"', html, re.I)
            if not m: m = re.search(r'"thumbnail_src"\s*:\s*"(https://[^"]+)"', html)
            if not m: m = re.search(r'"display_url"\s*:\s*"(https://[^"]+)"', html)
            if m:
                t2 = m.group(1).replace("\u0026", "&")
                if t2.startswith("http"): thumb = t2
        ...
        if own_uid and creator and own_uid in creator.replace("@", ""): continue
        ...
        rows.append({
            ...
            "description": desc_text or "", "creator": creator or "",
            "thumbnail": thumb or "", "posted_on": posted_on or "",
            "views": views, "likes": likes, "engagement": eng or 0,
