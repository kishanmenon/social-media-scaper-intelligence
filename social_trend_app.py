"""
social_trend_app.py v12
Key fixes:
- Implemented Dual-Extraction: Grabs thumbnails and descriptions directly from the IG grid to bypass individual page login walls.
- Strictly uses reference script's regex logic for parsing views/likes.
- Strictly uses reference script's BU categorization, renamed solely to "Category".
- Added fallback timestamps so posts don't disappear from the UI time tabs.
"""
...
def get_meta(html, prop_name):
    m = re.search(rf'<meta[^>]+(?:property|name)=["\']{prop_name}["\'][^>]+content=["\']([^"\']+)["\']', html, re.I)
    if m: return m.group(1)
    m = re.search(rf'<meta[^>]+content=["\']([^"\']+)["\'][^>]+(?:property|name)=["\']{prop_name}["\']', html, re.I)
    if m: return m.group(1)
    return None
...
async def scrape_ig(ctx, tag, limit=15):
    rows=[]
    reel_data=[]; page=await ctx.new_page()
    try:
        ...
        # EXTRACTION PHASE 1: Grab alt text (description) and src (thumbnail) directly from grid
        ...
        for el in links[:limit*2]:
            ...
            reel_data.append({
                "url": fmt("https://www.instagram.com", href),
                "grid_alt": grid_alt,
                "grid_thumb": grid_thumb
            })
            if len(reel_data) >= limit: break
    except: pass
    finally:
        try: await page.close()
        except: pass

    # EXTRACTION PHASE 2: Open individual pages for Views/Likes
    for item in reel_data:
        url = item["url"]
        rp = await ctx.new_page()
        
        # Fallbacks from Grid Phase
        views = likes = creator = None
        desc_text = item["grid_alt"]
        thumb = item["grid_thumb"]
        title = item["grid_alt"][:100] if item["grid_alt"] else f"#{tag} reel"
        posted_on = datetime.now().isoformat() # Fallback date so tabs don't break

        try:
            await rp.goto(url, wait_until="domcontentloaded", timeout=15000)
            await asyncio.sleep(1.5) 
            html=await rp.content()

            # Views/Likes using robust meta parser based on your script
            desc = get_meta(html, "og:description")
            if desc:
            ...
            page_thumb = get_meta(html, "og:image")
            if page_thumb: thumb = page_thumb.replace("&amp;", "&")

            # Better Date if available
            page_date = get_meta(html, "article:published_time")
            if page_date: 
                posted_on = page_date
            else:
        ...
        # Categorize using grid text or page text
        final_cat = classify_category(f"{title} {desc_text or ''} {tag}")
        
        rows.append({
            "platform":"Instagram", "content_type":"Reel",
            "hashtag":f"#{tag}", "url":url, "title":title.replace('\n', ' ').strip(),
            "description":desc_text.replace('\n', ' ').strip() if desc_text else "", 
...
# ── VIEW FILTERS ──────────────────────────────────────────────────────────────
st.markdown("---")
# Single Category filter based on Reference Rules
